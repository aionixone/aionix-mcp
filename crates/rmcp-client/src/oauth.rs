...
//!
//! If the keyring is not available or fails, we fall back to AIONIX_HOME/.credentials.json which is consistent with other coding CLI agents.
...
use crate::find_aionix_home::find_aionix_home;

const KEYRING_SERVICE: &str = "Aionix MCP Credentials";
...
/// Determine where Aionix should store and read MCP credentials.
#[derive(Debug, Default, Copy, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "lowercase")]
pub enum OAuthCredentialsStoreMode {
    /// `Keyring` when available; otherwise, `File`.
    /// Credentials stored in the keyring will only be readable by Aionix unless the user explicitly grants access via OS-level keyring access.
    #[default]
    Auto,
    /// AIONIX_HOME/.credentials.json
    /// This file will be readable to Aionix and other applications running as the same user.
    File,
...
mod tests {
    ...
    struct TempAionixHome {
        _guard: MutexGuard<'static, ()>,
        _override: AionixHomeOverrideGuard,
    ...
    impl TempAionixHome {
        fn new() -> Self {
            ...
            let dir = tempdir().expect("create AIONIX_HOME temp dir");
            let override_guard = set_aionix_home_override(dir.path().to_path_buf());
            Self {
                _guard: guard,
