...
use aionix_rmcp_client::RmcpClient;
...
const RESOURCE_URI: &str = "memo://aionix/example-note";

fn stdio_server_bin() -> anyhow::Result<PathBuf> {
    let build = CargoBuild::new()
        .package("aionix-rmcp-client")
...
fn init_params() -> InitializeRequestParams {
    InitializeRequestParams {
        ...
        client_info: Implementation {
            name: "aionix-test".into(),
            version: "0.0.0-test".into(),
            title: Some("Aionix rmcp resource test".into()),
            user_agent: None,
        },
...
async fn rmcp_client_can_list_and_read_resources() -> anyhow::Result<()> {
    ...
    assert_eq!(
        ...
        ListResourceTemplatesResult {
            ...
            resource_templates: vec![ResourceTemplate {
                annotations: None,
                description: Some(
                    "Template for memo://aionix/{slug} resources used in tests.".to_string()
                ),
                mime_type: Some("text/plain".to_string()),
                name: "aionix-memo".to_string(),
                title: Some("Aionix Memo".to_string()),
                uri_template: "memo://aionix/{slug}".to_string(),
            }],
        }
    );
    ...
    let expected = TextResourceContents {
